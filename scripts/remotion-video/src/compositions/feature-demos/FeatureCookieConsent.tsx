/**
 * FeatureCookieConsent — feature p38 — 1280x720, 15s @ 30fps, silent, loop-friendly.
 *
 * Story: Norwegian ekomloven + GDPR made aggressive banners force blind
 * "accept all" — legally required but practically meaningless →
 * CookieConsentContext (React Context) shows a minimal banner once,
 * persists choice to localStorage → TrackingContext gates every GTM tag
 * on isAccepted, no exceptions → /informasjonskapsler gives full
 * transparency → 100% compliance, zero UX compromise.
 */
...
// ── Layout ──────────────────────────────────────────────────────────
const TOP = { x: 440, y: 50, w: 400, h: 88, label: "Norwegian ekomloven + GDPR" };
const TOP_B: Pt = { x: TOP.x + TOP.w / 2, y: TOP.y + TOP.h };

const RIGHT1 = { x: 850, y: 210, w: 300, h: 78, label: "aggressive full-screen banners" };
const RIGHT1_T: Pt = { x: RIGHT1.x + RIGHT1.w / 2, y: RIGHT1.y };

const LEFT1 = { x: 130, y: 210, w: 280, h: 78, label: "CookieConsentContext" };
...
export const FeatureCookieConsent: React.FC = () => {
  ...
  // ── Beat 1 (0–116): aggressive banners force blind consent, meaningless ──
  ...
  // ── Beat 2 (128–232): minimal banner, choice persisted to localStorage ──
  ...
  // ── Beat 3 (246–340): TrackingContext gates every tag; full transparency page ──
  ...
  return (
    <AbsoluteFill style={{ fontFamily }}>
      ...
      <SchemaNode {...RIGHT1} state="danger" lit={rightLit} opacity={rightOp} label={RIGHT1.label} fontSize={16}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>forces blind "accept all"</div>
      </SchemaNode>
      <Badge x={RIGHT1.x + RIGHT1.w / 2 - 16} y={RIGHT1.y - 34} kind="cross" scale={xScale} opacity={xScale} size={32} />
      <Pill x={TOP.x - 30} y={TOP.y - 44} text="legally required but practically meaningless" color={T.danger} opacity={pill1Op} fontSize={16} />
      ...
      <SchemaNode {...MID} state="accent" lit={midLit} opacity={midOp} label="minimal banner, shown once" fontSize={19}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2, textAlign: "center" }}>choice persisted to localStorage</div>
      </SchemaNode>
      <Token pts={[LEFT1_B, MID_T]} t={tEf} opacity={tEfVis * lf} />

      <SchemaNode {...BOTLEFT} state="success" lit={botLeftLit} opacity={botLeftOp} label="TrackingContext gatekeeper" fontSize={16}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>checks isAccepted first</div>
      </SchemaNode>
      <SchemaNode {...BOTRIGHT} state="success" lit={botRightLit} opacity={botRightOp} label="/informasjonskapsler page" fontSize={17}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>full transparent policy</div>
      </SchemaNode>
      ...
      <Pill x={BOTLEFT.x - 10} y={BOTLEFT.y + BOTLEFT.h + 14} text="opt-out = full, unimpeded site access" color={T.success} opacity={pill3Op} fontSize={15} />

      <Caption x={90} y={648} w={1100} text="Aggressive banners force blind consent — legally required, practically meaningless" color={T.danger} opacity={cap1} fontSize={21} weight={600} />
      <Caption x={90} y={648} w={1100} text="A minimal banner persists choice to localStorage, shown exactly once" color={T.text} opacity={cap2} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="TrackingContext gates every GTM tag on real consent state, no exceptions" color={T.success} opacity={cap3} fontSize={22} weight={600} />

      <div style={{ position: "absolute", left: 0, top: 598, width: 1280, textAlign: "center", opacity: metricOp }}>
        <div style={{ fontSize: 22, fontWeight: 600, color: T.muted }}>Zero UX compromise</div>
      </div>
      <div
        style={{
          position: "absolute",
      ...
      >
        ...
        <div style={{ fontSize: 27, fontWeight: 600, color: T.success }}>100% ekomloven/GDPR compliance</div>
      </div>
    </AbsoluteFill>
  );
};
