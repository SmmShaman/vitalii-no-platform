/**
 * FeatureGtmIntegrationHub — feature p37 — 1280x720, 15s @ 30fps, silent, loop-friendly.
 *
 * Story: 4 separate analytics scripts (GA4, Meta Pixel, LinkedIn,
 * custom) meant touching multiple codebases per event → one
 * @next/third-parties GoogleTagManager (GTM-5XBL8L8S) becomes the sole
 * entry point → a typed TrackingContext (trackPageView, trackArticleView,
 * trackShare...) pushes to window.dataLayer, gated by cookie consent →
 * GTM dispatches to all platforms → dev overhead for new events down 75%.
 */
...
// ── Layout ──────────────────────────────────────────────────────────
const TOP = { x: 440, y: 50, w: 400, h: 88, label: "4 analytics platforms" };
const TOP_B: Pt = { x: TOP.x + TOP.w / 2, y: TOP.y + TOP.h };

const RIGHT1 = { x: 850, y: 210, w: 300, h: 78, label: "4 separate scripts" };
const RIGHT1_T: Pt = { x: RIGHT1.x + RIGHT1.w / 2, y: RIGHT1.y };

const LEFT1 = { x: 130, y: 210, w: 280, h: 78, label: "GoogleTagManager (GTM)" };
...
export const FeatureGtmIntegrationHub: React.FC = () => {
  ...
  // ── Beat 1 (0–116): 4 scripts, each event touches multiple codebases ──
  ...
  // ── Beat 2 (128–232): one GTM script + typed TrackingContext hooks ──
  ...
  // ── Beat 3 (246–340): single dataLayer.push(), gated by cookie consent ──
  ...
  return (
    <AbsoluteFill style={{ fontFamily }}>
      ...
      <SchemaNode {...TOP} state="idle" lit={topLit} opacity={topOp} label={TOP.label} fontSize={22}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>GA4 · Meta Pixel · LinkedIn · custom</div>
      </SchemaNode>
      ...
      <SchemaNode {...RIGHT1} state="danger" lit={rightLit} opacity={rightOp} label={RIGHT1.label} fontSize={19}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>bloated weight, maintenance pain</div>
      </SchemaNode>
      <Badge x={RIGHT1.x + RIGHT1.w / 2 - 16} y={RIGHT1.y - 34} kind="cross" scale={xScale} opacity={xScale} size={32} />
      <Pill x={TOP.x - 30} y={TOP.y - 44} text="every new event touched multiple codebases" color={T.danger} opacity={pill1Op} fontSize={16} />

      <SchemaNode {...LEFT1} state="accent" lit={leftLit} opacity={leftOp} label={LEFT1.label} fontSize={17}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>@next/third-parties</div>
      </SchemaNode>
      <Token pts={[TOP_B, LEFT1_T]} t={tCe} opacity={tCeVis * lf} />
      <SchemaNode {...MID} state="accent" lit={midLit} opacity={midOp} label="TrackingContext hooks" fontSize={19}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2, textAlign: "center" }}>trackPageView · trackShare · trackArticleView</div>
      </SchemaNode>
      <Token pts={[LEFT1_B, MID_T]} t={tEf} opacity={tEfVis * lf} />

      <SchemaNode {...BOTLEFT} state="success" lit={botLeftLit} opacity={botLeftOp} label="dataLayer.push()" fontSize={19}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>single typed entry point</div>
      </SchemaNode>
      <SchemaNode {...BOTRIGHT} state="amber" lit={botRightLit} opacity={botRightOp} label="cookie consent gate" fontSize={19}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>GDPR before any tag fires</div>
      </SchemaNode>
      ...
      <Pill x={BOTLEFT.x - 10} y={BOTLEFT.y + BOTLEFT.h + 14} text="GTM dispatches to GA4, Pixel, LinkedIn" color={T.success} opacity={pill3Op} fontSize={15} />

      <Caption x={90} y={648} w={1100} text="Four separate analytics scripts meant touching multiple codebases per event" color={T.danger} opacity={cap1} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="One GTM script and a typed TrackingContext become the single entry point" color={T.text} opacity={cap2} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="Every event is a single dataLayer.push(), gated by cookie consent" color={T.amber} opacity={cap3} fontSize={22} weight={600} />

      <div style={{ position: "absolute", left: 0, top: 598, width: 1280, textAlign: "center", opacity: metricOp }}>
        <div style={{ fontSize: 22, fontWeight: 600, color: T.muted }}>Real-time debug console, no redeploys</div>
      </div>
      <div
        style={{
          position: "absolute",
      ...
      >
        ...
        <div style={{ fontSize: 27, fontWeight: 600, color: T.success }}>Dev overhead for new events down 75%</div>
      </div>
    </AbsoluteFill>
  );
};
