/**
 * FeatureApiRetryLogic — feature p42 — 1280x720, 15s @ 30fps, silent, loop-friendly.
 *
 * Story: transient 5xx errors from critical APIs halted GitHub Actions
 * pipelines feeding vitalii.no from 6 Telegram channels + RSS, meaning
 * manual restarts and missed windows → call_api_with_retry (bash)
 * captures body + status via curl -w, parsed by jq: 3 attempts, 15s
 * delay → 5xx retries, 4xx reported immediately, no wasted retries on
 * real bugs → 95%+ succeed on the second attempt.
 */
...
// ── Layout ──────────────────────────────────────────────────────────
const TOP = { x: 440, y: 50, w: 400, h: 88, label: "6 Telegram channels + RSS" };
const TOP_B: Pt = { x: TOP.x + TOP.w / 2, y: TOP.y + TOP.h };

const RIGHT1 = { x: 850, y: 210, w: 300, h: 78, label: "transient 5xx halts pipeline" };
const RIGHT1_T: Pt = { x: RIGHT1.x + RIGHT1.w / 2, y: RIGHT1.y };

const LEFT1 = { x: 130, y: 210, w: 280, h: 78, label: "call_api_with_retry (bash)" };
...
export const FeatureApiRetryLogic: React.FC = () => {
  ...
  // ── Beat 1 (0–116): a single transient 5xx halted the whole workflow ──
  ...
  // ── Beat 2 (128–232): curl -w captures status, jq parses, 3 attempts/15s ──
  ...
  // ── Beat 3 (246–340): 5xx retries; 4xx reported immediately, never retried ──
  ...
  return (
    <AbsoluteFill style={{ fontFamily }}>
      ...
      <Connector pts={[MID_B, BOTRIGHT_T]} color={T.danger} width={2.5} progress={tFc} opacity={0.8 * tFcVis * lf} />
      ...
      <SchemaNode {...RIGHT1} state="danger" lit={rightLit} opacity={rightOp} label={RIGHT1.label} fontSize={17}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>manual restarts, missed windows</div>
      </SchemaNode>
      <Badge x={RIGHT1.x + RIGHT1.w / 2 - 16} y={RIGHT1.y - 34} kind="cross" scale={xScale} opacity={xScale} size={32} />
      <Pill x={TOP.x - 30} y={TOP.y - 44} text="one API hiccup meant hours lost" color={T.danger} opacity={pill1Op} fontSize={16} />
      ...
      <SchemaNode {...MID} state="accent" lit={midLit} opacity={midOp} label="curl -w %{http_code} -> jq" fontSize={17}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2, textAlign: "center" }}>3 attempts, 15s delay</div>
      </SchemaNode>
      <Token pts={[LEFT1_B, MID_T]} t={tEf} opacity={tEfVis * lf} />

      <SchemaNode {...BOTLEFT} state="amber" lit={botLeftLit} opacity={botLeftOp} label="5xx -> retry" fontSize={19}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>server errors only</div>
      </SchemaNode>
      <SchemaNode {...BOTRIGHT} state="danger" lit={botRightLit} opacity={botRightOp} label="4xx -> report immediately" fontSize={17}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>never retries client errors</div>
      </SchemaNode>
      <Token pts={[MID_B, BOTLEFT_T]} t={tFl} color={T.amber} opacity={tFlVis * lf} />
      <Token pts={[MID_B, BOTRIGHT_T]} t={tFc} color={T.danger} opacity={tFcVis * lf} />
      <Pill x={BOTLEFT.x - 10} y={BOTLEFT.y + BOTLEFT.h + 14} text="structured error logging, no wasted retries" color={T.amber} opacity={pill3Op} fontSize={15} />

      <Caption x={90} y={648} w={1100} text="A single transient 5xx from any critical API halted the whole workflow" color={T.danger} opacity={cap1} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="call_api_with_retry captures body and status via curl, parsed by jq" color={T.text} opacity={cap2} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="5xx errors retry three times; 4xx client errors are reported immediately" color={T.amber} opacity={cap3} fontSize={22} weight={600} />

      <div style={{ position: "absolute", left: 0, top: 598, width: 1280, textAlign: "center", opacity: metricOp }}>
        <div style={{ fontSize: 22, fontWeight: 600, color: T.muted }}>95%+ succeed on the second attempt</div>
      </div>
      <div
        style={{
          position: "absolute",
      ...
      >
        ...
        <div style={{ fontSize: 27, fontWeight: 600, color: T.success }}>Manual intervention effectively eliminated</div>
      </div>
    </AbsoluteFill>
  );
};
