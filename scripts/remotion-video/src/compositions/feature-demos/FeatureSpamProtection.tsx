 ...
 *
 * Story: 50+ spam submissions/day (SEO offers, casino, crypto) flooded the
 * contact form → send-contact-email (Deno) runs a 3-tier defense: hidden
 * email_confirm honeypot, Date.now() < 3s reject, in-memory IP rate limit
 * (5 req / 10 min) → all checks run before Resend API dispatch → 0-1
 * spam/day remain, zero false positives, no CAPTCHA needed.
 */
...
// ── Layout ──────────────────────────────────────────────────────────
const TOP = { x: 440, y: 50, w: 400, h: 88, label: "Contact form" };
const TOP_B: Pt = { x: TOP.x + TOP.w / 2, y: TOP.y + TOP.h };

const RIGHT1 = { x: 850, y: 210, w: 300, h: 78, label: "50+ spam submissions/day" };
const RIGHT1_T: Pt = { x: RIGHT1.x + RIGHT1.w / 2, y: RIGHT1.y };

const LEFT1 = { x: 130, y: 210, w: 280, h: 78, label: "Tier 1: email_confirm honeypot" };
...
export const FeatureSpamProtection: React.FC = () => {
  ...
  // ── Beat 1 (0–116): contact form → 50+ spam submissions/day ──
  ...
  // ── Beat 2 (128–232): honeypot + 3-second timestamp reject ──
  ...
  // ── Beat 3 (246–340): Tier 3 IP rate limit → Resend dispatch only after all checks ──
  ...
  return (
    <AbsoluteFill style={{ fontFamily }}>
      ...
      <SchemaNode {...RIGHT1} state="danger" lit={rightLit} opacity={rightOp} label={RIGHT1.label} fontSize={18}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>SEO offers, casino, crypto</div>
      </SchemaNode>
      <Badge x={RIGHT1.x + RIGHT1.w / 2 - 16} y={RIGHT1.y - 34} kind="cross" scale={xScale} opacity={xScale} size={32} />
      <Pill x={TOP.x - 20} y={TOP.y - 44} text="drowning in junk within the first week" color={T.danger} opacity={pill1Op} fontSize={16} />
      ...
      <SchemaNode {...MID} state="accent" lit={midLit} opacity={midOp} label="Tier 2: fill time < 3s" fontSize={19}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2, textAlign: "center" }}>Date.now() captured on render</div>
      </SchemaNode>
      <Token pts={[LEFT1_B, MID_T]} t={tEf} opacity={tEfVis * lf} />

      <SchemaNode {...BOTLEFT} state="amber" lit={botLeftLit} opacity={botLeftOp} label="Tier 3: IP rate limit" fontSize={18}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>5 req / 10 min window</div>
      </SchemaNode>
      <SchemaNode {...BOTRIGHT} state="success" lit={botRightLit} opacity={botRightOp} label="Resend API dispatch" fontSize={19}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>only after all 3 checks pass</div>
      </SchemaNode>
      ...
      <Pill x={BOTLEFT.x - 10} y={BOTLEFT.y + BOTLEFT.h + 14} text="in-memory Map tracks submissions per IP" color={T.amber} opacity={pill3Op} fontSize={15} />

      <Caption x={90} y={648} w={1100} text="Within the first week, 50+ spam submissions a day drowned the contact form" color={T.danger} opacity={cap1} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="A hidden honeypot field and a 3-second minimum fill time catch bots instantly" color={T.text} opacity={cap2} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="IP-based rate limiting blocks the rest, before any email is ever sent" color={T.amber} opacity={cap3} fontSize={22} weight={600} />

      <div style={{ position: "absolute", left: 0, top: 598, width: 1280, textAlign: "center", opacity: metricOp }}>
        <div style={{ fontSize: 22, fontWeight: 600, color: T.muted }}>0-1 spam/day remain, zero false positives</div>
      </div>
      <div
        style={{
          position: "absolute",
      ...
      >
        ...
        <div style={{ fontSize: 27, fontWeight: 600, color: T.success }}>98% reduction, no CAPTCHA needed</div>
      </div>
    </AbsoluteFill>
  );
};
