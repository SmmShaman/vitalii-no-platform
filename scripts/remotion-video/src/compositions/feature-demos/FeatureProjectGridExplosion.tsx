/**
 * FeatureProjectGridExplosion — feature p29 — 1280x720, 15s @ 30fps, silent, loop-friendly.
 *
 * Story: visitors rarely scrolled past 2-3 of 9+ projects in the carousel,
 * a black hole for most of the work → useHoverTimer tracks a deliberate
 * 3-second hover, then gsap.timeline() explodes cards into a CSS Grid
 * (grid-template-columns adapts to count) → each card staggers in by
 * index*0.05s with backOut easing, one of 5 neon gradients → est. 30-50%
 * more time exploring projects.
 */
...
// ── Layout ──────────────────────────────────────────────────────────
const TOP = { x: 440, y: 50, w: 400, h: 88, label: "Project carousel, 9+ projects" };
const TOP_B: Pt = { x: TOP.x + TOP.w / 2, y: TOP.y + TOP.h };

const RIGHT1 = { x: 850, y: 210, w: 300, h: 78, label: "2-3 seen, rest hidden" };
const RIGHT1_T: Pt = { x: RIGHT1.x + RIGHT1.w / 2, y: RIGHT1.y };

const LEFT1 = { x: 130, y: 210, w: 280, h: 78, label: "useHoverTimer hook" };
...
export const FeatureProjectGridExplosion: React.FC = () => {
  ...
  // ── Beat 1 (0–116): carousel is a black hole, most projects stay hidden ──
  ...
  // ── Beat 2 (128–232): 3s hover -> gsap.timeline() explosion ──
  ...
  // ── Beat 3 (246–340): staggered CSS Grid explosion, 5 neon gradients ──
  ...
  return (
    <AbsoluteFill style={{ fontFamily }}>
      ...
      <SchemaNode {...RIGHT1} state="danger" lit={rightLit} opacity={rightOp} label={RIGHT1.label} fontSize={19}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>a black hole for 6+ projects</div>
      </SchemaNode>
      <Badge x={RIGHT1.x + RIGHT1.w / 2 - 16} y={RIGHT1.y - 34} kind="cross" scale={xScale} opacity={xScale} size={32} />
      <Pill x={TOP.x - 30} y={TOP.y - 44} text="missed opportunity to showcase diverse skills" color={T.danger} opacity={pill1Op} fontSize={16} />
      ...
      <SchemaNode {...MID} state="accent" lit={midLit} opacity={midOp} label="gsap.timeline() on hover end" fontSize={18}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2, textAlign: "center" }}>3-second deliberate hover</div>
      </SchemaNode>
      <Token pts={[LEFT1_B, MID_T]} t={tEf} opacity={tEfVis * lf} />

      <SchemaNode {...BOTLEFT} state="success" lit={botLeftLit} opacity={botLeftOp} label="CSS Grid explosion" fontSize={19}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>repeat(3, 1fr), stagger 0.05s</div>
      </SchemaNode>
      <SchemaNode {...BOTRIGHT} state="success" lit={botRightLit} opacity={botRightOp} label="5 neon gradient schemes" fontSize={18}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>backOut easing per card</div>
      </SchemaNode>
      ...
      <Pill x={BOTLEFT.x - 10} y={BOTLEFT.y + BOTLEFT.h + 14} text="progress bar hints at the transition" color={T.success} opacity={pill3Op} fontSize={15} />

      <Caption x={90} y={648} w={1100} text="Visitors rarely scrolled past 2-3 of 9+ projects — most work stayed hidden" color={T.danger} opacity={cap1} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="A 3-second hover timer triggers a GSAP explosion into a full CSS Grid" color={T.text} opacity={cap2} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="Cards stagger in with backOut easing across 5 neon gradient schemes" color={T.success} opacity={cap3} fontSize={22} weight={600} />

      <div style={{ position: "absolute", left: 0, top: 598, width: 1280, textAlign: "center", opacity: metricOp }}>
        <div style={{ fontSize: 22, fontWeight: 600, color: T.muted }}>3-second hover = the "wow" is earned</div>
      </div>
      <div
        style={{
          position: "absolute",
      ...
      >
        ...
        <div style={{ fontSize: 27, fontWeight: 600, color: T.success }}>Est. 30-50% more time exploring projects</div>
      </div>
    </AbsoluteFill>
  );
};
