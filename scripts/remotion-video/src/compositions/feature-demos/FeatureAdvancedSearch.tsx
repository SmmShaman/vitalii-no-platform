/**
 * FeatureAdvancedSearch — feature p33 — 1280x720, 15s @ 30fps, silent, loop-friendly.
 *
 * Story: 200+ articles turned basic keyword search into a frustrating
 * dead end, a digital haystack with a teaspoon → a 300ms-debounced
 * full-text search combines free text, multi-select tags, date ranges,
 * and content types → getYouTubeThumbnail() renders hqdefault thumbnails
 * with a play overlay in a masonry grid, plus a frequency-weighted tag
 * cloud → the search page became the primary content discovery hub.
 */
...
// ── Layout ──────────────────────────────────────────────────────────
const TOP = { x: 440, y: 50, w: 400, h: 88, label: "200+ articles" };
const TOP_B: Pt = { x: TOP.x + TOP.w / 2, y: TOP.y + TOP.h };

const RIGHT1 = { x: 850, y: 210, w: 300, h: 78, label: "basic keyword search" };
const RIGHT1_T: Pt = { x: RIGHT1.x + RIGHT1.w / 2, y: RIGHT1.y };

const LEFT1 = { x: 130, y: 210, w: 280, h: 78, label: "full-text, 300ms debounce" };
...
export const FeatureAdvancedSearch: React.FC = () => {
  ...
  // ── Beat 1 (0–116): 200+ articles, basic keyword search is a dead end ──
  ...
  // ── Beat 2 (128–232): debounced full-text search + multi-filter ──
  ...
  // ── Beat 3 (246–340): masonry grid + YouTube thumbnails, weighted tag cloud ──
  ...
  return (
    <AbsoluteFill style={{ fontFamily }}>
      ...
      <SchemaNode {...RIGHT1} state="danger" lit={rightLit} opacity={rightOp} label={RIGHT1.label} fontSize={19}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>digital haystack, teaspoon</div>
      </SchemaNode>
      <Badge x={RIGHT1.x + RIGHT1.w / 2 - 16} y={RIGHT1.y - 34} kind="cross" scale={xScale} opacity={xScale} size={32} />
      <Pill x={TOP.x - 40} y={TOP.y - 44} text="can't filter by tag + date + type at once" color={T.danger} opacity={pill1Op} fontSize={16} />
      ...
      <SchemaNode {...MID} state="accent" lit={midLit} opacity={midOp} label="multi-filter system" fontSize={20}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2, textAlign: "center" }}>tags · date ranges · content types</div>
      </SchemaNode>
      <Token pts={[LEFT1_B, MID_T]} t={tEf} opacity={tEfVis * lf} />

      <SchemaNode {...BOTLEFT} state="success" lit={botLeftLit} opacity={botLeftOp} label="masonry grid" fontSize={19}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>getYouTubeThumbnail() hqdefault</div>
      </SchemaNode>
      <SchemaNode {...BOTRIGHT} state="success" lit={botRightLit} opacity={botRightOp} label="frequency-weighted tag cloud" fontSize={17}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>header search expands right-to-left</div>
      </SchemaNode>
      ...
      <Pill x={BOTLEFT.x - 10} y={BOTLEFT.y + BOTLEFT.h + 14} text="custom play overlay on video thumbnails" color={T.success} opacity={pill3Op} fontSize={15} />

      <Caption x={90} y={648} w={1100} text="200+ articles turned basic keyword search into a frustrating dead end" color={T.danger} opacity={cap1} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="A debounced full-text search combines free text, tags, dates, and content type" color={T.text} opacity={cap2} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="Rich media renders in a masonry grid with a frequency-weighted tag cloud" color={T.success} opacity={cap3} fontSize={22} weight={600} />

      <div style={{ position: "absolute", left: 0, top: 598, width: 1280, textAlign: "center", opacity: metricOp }}>
        <div style={{ fontSize: 22, fontWeight: 600, color: T.muted }}>Primary content discovery hub</div>
      </div>
      <div
        style={{
          position: "absolute",
      ...
      >
        ...
        <div style={{ fontSize: 27, fontWeight: 600, color: T.success }}>Effortless navigation across 200+ articles</div>
      </div>
    </AbsoluteFill>
  );
};
