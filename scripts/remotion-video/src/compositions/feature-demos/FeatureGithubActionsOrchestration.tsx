/**
 * FeatureGithubActionsOrchestration — feature p39 — 1280x720, 15s @ 30fps, silent, loop-friendly.
 *
 * Story: managing 43 Edge Functions, scraping every 10 minutes, and 5+
 * minute video processing meant manual deploys and babysitting flaky
 * retries — hours lost daily, scaling impossible → 20+ .yml workflows
 * automate deploys, scraping, and video with a call_api_with_retry bash
 * pattern (3 attempts, 15s delay on 5xx) → concurrency groups and a jq
 * round-robin channel picker keep every run isolated → 24/7 autonomous
 * operation, zero-person ops team.
 */
...
// ── Layout ──────────────────────────────────────────────────────────
const TOP = { x: 440, y: 50, w: 400, h: 88, label: "43 Edge Functions" };
const TOP_B: Pt = { x: TOP.x + TOP.w / 2, y: TOP.y + TOP.h };

const RIGHT1 = { x: 850, y: 210, w: 300, h: 78, label: "manual deploys & babysitting" };
const RIGHT1_T: Pt = { x: RIGHT1.x + RIGHT1.w / 2, y: RIGHT1.y };

const LEFT1 = { x: 130, y: 210, w: 280, h: 78, label: "20+ .yml workflows" };
...
export const FeatureGithubActionsOrchestration: React.FC = () => {
  ...
  // ── Beat 1 (0–116): manual deploys and flaky retries, a critical bottleneck ──
  ...
  // ── Beat 2 (128–232): 20+ workflows automate deploys, scraping, video ──
  ...
  // ── Beat 3 (246–340): concurrency groups + round-robin channel picking ──
  ...
  return (
    <AbsoluteFill style={{ fontFamily }}>
      ...
      <SchemaNode {...TOP} state="idle" lit={topLit} opacity={topOp} label={TOP.label} fontSize={22}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>scrape 10 min, video 30 min</div>
      </SchemaNode>
      <Token pts={[TOP_B, RIGHT1_T]} t={tCi} color={T.danger} opacity={tCiVis * lf} />
      <SchemaNode {...RIGHT1} state="danger" lit={rightLit} opacity={rightOp} label={RIGHT1.label} fontSize={16}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>hours lost, scaling impossible</div>
      </SchemaNode>
      <Badge x={RIGHT1.x + RIGHT1.w / 2 - 16} y={RIGHT1.y - 34} kind="cross" scale={xScale} opacity={xScale} size={32} />
      <Pill x={TOP.x - 20} y={TOP.y - 44} text="a critical bottleneck for a solo developer" color={T.danger} opacity={pill1Op} fontSize={16} />
      ...
      <SchemaNode {...MID} state="accent" lit={midLit} opacity={midOp} label="call_api_with_retry" fontSize={19}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2, textAlign: "center" }}>MAX_RETRIES=3, 15s delay on 5xx</div>
      </SchemaNode>
      <Token pts={[LEFT1_B, MID_T]} t={tEf} opacity={tEfVis * lf} />

      <SchemaNode {...BOTLEFT} state="success" lit={botLeftLit} opacity={botLeftOp} label="concurrency groups" fontSize={17}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>scraper-workflow prevents overlap</div>
      </SchemaNode>
      <SchemaNode {...BOTRIGHT} state="success" lit={botRightLit} opacity={botRightOp} label="jq round-robin picker" fontSize={18}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>8 RSS sources per batch run</div>
      </SchemaNode>
      ...
      <Pill x={BOTLEFT.x - 10} y={BOTLEFT.y + BOTLEFT.h + 14} text="video processes every 30 minutes" color={T.success} opacity={pill3Op} fontSize={15} />

      <Caption x={90} y={648} w={1100} text="Manual deploys and flaky retries devoured hours, blocking any real scaling" color={T.danger} opacity={cap1} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="20+ GitHub Actions workflows automate deploys, scraping, and video processing" color={T.text} opacity={cap2} fontSize={21} weight={600} />
      <Caption x={90} y={648} w={1100} text="Concurrency groups and round-robin channel picking keep every run isolated" color={T.success} opacity={cap3} fontSize={22} weight={600} />

      <div style={{ position: "absolute", left: 0, top: 598, width: 1280, textAlign: "center", opacity: metricOp }}>
        <div style={{ fontSize: 22, fontWeight: 600, color: T.muted }}>24/7 autonomous operation</div>
      </div>
      <div
        style={{
          position: "absolute",
      ...
      >
        ...
        <div style={{ fontSize: 27, fontWeight: 600, color: T.success }}>Zero-person ops team, zero manual intervention</div>
      </div>
    </AbsoluteFill>
  );
};
