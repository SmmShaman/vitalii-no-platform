/**
 * FeatureNetlifyDeploySplit — feature p40 — 1280x720, 15s @ 30fps, silent, loop-friendly.
 *
 * Story: Netlify's auto-builds introduced env var drift vs local setup;
 * a tiny Edge Function fix used to trigger a full, unnecessary Next.js
 * rebuild → deployments split into deploy.yml (frontend only, npm run
 * build + netlify deploy --prod --dir=./.next) and a separate
 * deploy-supabase.yml watching only supabase/** → atomic backend
 * updates, no wasted rebuild → 10-15 min saved per non-frontend change.
 */
...
// ── Layout ──────────────────────────────────────────────────────────
const TOP = { x: 440, y: 50, w: 400, h: 88, label: "Push to main" };
const TOP_B: Pt = { x: TOP.x + TOP.w / 2, y: TOP.y + TOP.h };

const RIGHT1 = { x: 850, y: 210, w: 300, h: 78, label: "Netlify auto-build gamble" };
const RIGHT1_T: Pt = { x: RIGHT1.x + RIGHT1.w / 2, y: RIGHT1.y };

const LEFT1 = { x: 130, y: 210, w: 280, h: 78, label: "deploy.yml (frontend only)" };
...
export const FeatureNetlifyDeploySplit: React.FC = () => {
  ...
  // ── Beat 1 (0–116): tiny Edge Function fix used to trigger a full rebuild ──
  ...
  // ── Beat 2 (128–232): deploy.yml exclusively owns frontend builds ──
  ...
  // ── Beat 3 (246–340): deploy-supabase.yml watches only supabase/**, atomic ──
  ...
  const botLeftLit = interpolate(frame, [274, 296, 330, 350], [0, 0.6, 0.6, 0.16], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) * lf;
  ...
  return (
    <AbsoluteFill style={{ fontFamily }}>
      ...
      <SchemaNode {...TOP} state="idle" lit={topLit} opacity={topOp} label={TOP.label} fontSize={24} />
      <Token pts={[TOP_B, RIGHT1_T]} t={tCi} color={T.danger} opacity={tCiVis * lf} />
      <SchemaNode {...RIGHT1} state="danger" lit={rightLit} opacity={rightOp} label={RIGHT1.label} fontSize={18}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>env var drift, phantom bugs</div>
      </SchemaNode>
      <Badge x={RIGHT1.x + RIGHT1.w / 2 - 16} y={RIGHT1.y - 34} kind="cross" scale={xScale} opacity={xScale} size={32} />
      <Pill x={TOP.x - 20} y={TOP.y - 44} text="tiny Edge Function fix triggered full rebuild" color={T.danger} opacity={pill1Op} fontSize={16} />

      <SchemaNode {...LEFT1} state="accent" lit={leftLit} opacity={leftOp} label={LEFT1.label} fontSize={17} />
      <Token pts={[TOP_B, LEFT1_T]} t={tCe} opacity={tCeVis * lf} />
      <SchemaNode {...MID} state="accent" lit={midLit} opacity={midOp} label="netlify deploy --prod" fontSize={19}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2, textAlign: "center" }}>NETLIFY_AUTH_TOKEN + SITE_ID</div>
      </SchemaNode>
      <Token pts={[LEFT1_B, MID_T]} t={tEf} opacity={tEfVis * lf} />

      <SchemaNode {...BOTLEFT} state="amber" lit={botLeftLit} opacity={botLeftOp} label="deploy-supabase.yml" fontSize={17}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>watches supabase/** only</div>
      </SchemaNode>
      <SchemaNode {...BOTRIGHT} state="success" lit={botRightLit} opacity={botRightOp} label="atomic backend updates" fontSize={17}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>no unnecessary Next.js rebuild</div>
      </SchemaNode>
      ...
      <Pill x={BOTLEFT.x - 10} y={BOTLEFT.y + BOTLEFT.h + 14} text="GitHub Actions = single source of truth" color={T.success} opacity={pill3Op} fontSize={15} />

      <Caption x={90} y={648} w={1100} text="A tiny Edge Function fix used to trigger a full, unnecessary Next.js rebuild" color={T.danger} opacity={cap1} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="deploy.yml now owns frontend builds exclusively, via the Netlify CLI" color={T.text} opacity={cap2} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="A separate deploy-supabase.yml handles functions and migrations atomically" color={T.success} opacity={cap3} fontSize={22} weight={600} />

      <div style={{ position: "absolute", left: 0, top: 598, width: 1280, textAlign: "center", opacity: metricOp }}>
        <div style={{ fontSize: 22, fontWeight: 600, color: T.muted }}>10-15 min saved per non-frontend change</div>
      </div>
      <div
        style={{
          position: "absolute",
      ...
      >
        ...
        <div style={{ fontSize: 27, fontWeight: 600, color: T.success }}>Predictable, debuggable, trusted deploys</div>
      </div>
    </AbsoluteFill>
  );
};
