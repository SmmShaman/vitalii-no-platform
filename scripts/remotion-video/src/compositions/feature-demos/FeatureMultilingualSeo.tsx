/**
 * FeatureMultilingualSeo — feature p35 — 1280x720, 15s @ 30fps, silent, loop-friendly.
 *
 * Story: publishing in EN, NO, UA without explicit signals let Google
 * confuse which language version to show, risking duplicate content
 * penalties → app/sitemap.ts dynamically injects hreflang alternates for
 * every article, every language, with site-wide canonical URLs →
 * JSON-LD (BlogPosting/NewsArticle) carries inLanguage, og:locale set
 * per language → zero duplicate content warnings, correct indexing
...
// ── Layout ──────────────────────────────────────────────────────────
const TOP = { x: 440, y: 50, w: 400, h: 88, label: "Content in EN, NO, UA" };
const TOP_B: Pt = { x: TOP.x + TOP.w / 2, y: TOP.y + TOP.h };

const RIGHT1 = { x: 850, y: 210, w: 300, h: 78, label: "Google confuses versions" };
const RIGHT1_T: Pt = { x: RIGHT1.x + RIGHT1.w / 2, y: RIGHT1.y };

const LEFT1 = { x: 130, y: 210, w: 280, h: 78, label: "app/sitemap.ts" };
...
export const FeatureMultilingualSeo: React.FC = () => {
  ...
  // ── Beat 1 (0–116): without explicit signals, Google confuses language versions ──
  ...
  // ── Beat 2 (128–232): dynamic sitemap injects hreflang alternates, canonical URLs ──
  ...
  // ── Beat 3 (246–340): JSON-LD inLanguage + og:locale per language ──
  ...
  return (
    <AbsoluteFill style={{ fontFamily }}>
      ...
      <SchemaNode {...RIGHT1} state="danger" lit={rightLit} opacity={rightOp} label={RIGHT1.label} fontSize={18}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>duplicate content risk</div>
      </SchemaNode>
      <Badge x={RIGHT1.x + RIGHT1.w / 2 - 16} y={RIGHT1.y - 34} kind="cross" scale={xScale} opacity={xScale} size={32} />
      <Pill x={TOP.x - 30} y={TOP.y - 44} text="NO content could outrank EN searches" color={T.danger} opacity={pill1Op} fontSize={16} />

      <SchemaNode {...LEFT1} state="accent" lit={leftLit} opacity={leftOp} label={LEFT1.label} fontSize={19} />
      <Token pts={[TOP_B, LEFT1_T]} t={tCe} opacity={tCeVis * lf} />
      <SchemaNode {...MID} state="accent" lit={midLit} opacity={midOp} label="hreflang='en|no|uk'" fontSize={19}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2, textAlign: "center" }}>canonical URLs site-wide</div>
      </SchemaNode>
      <Token pts={[LEFT1_B, MID_T]} t={tEf} opacity={tEfVis * lf} />

      <SchemaNode {...BOTLEFT} state="success" lit={botLeftLit} opacity={botLeftOp} label="JSON-LD schemas" fontSize={19}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>inLanguage property</div>
      </SchemaNode>
      <SchemaNode {...BOTRIGHT} state="success" lit={botRightLit} opacity={botRightOp} label="og:locale per language" fontSize={18}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>accurate social previews</div>
      </SchemaNode>
      ...
      <Pill x={BOTLEFT.x - 10} y={BOTLEFT.y + BOTLEFT.h + 14} text="rich snippets, per language" color={T.success} opacity={pill3Op} fontSize={15} />

      <Caption x={90} y={648} w={1100} text="Without explicit signals, Google confused which language version to show" color={T.danger} opacity={cap1} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="A dynamic sitemap injects hreflang alternates for every article, every language" color={T.text} opacity={cap2} fontSize={21} weight={600} />
      <Caption x={90} y={648} w={1100} text="JSON-LD schemas and og:locale tags make each language explicit end to end" color={T.success} opacity={cap3} fontSize={22} weight={600} />

      <div style={{ position: "absolute", left: 0, top: 598, width: 1280, textAlign: "center", opacity: metricOp }}>
        <div style={{ fontSize: 22, fontWeight: 600, color: T.muted }}>Zero duplicate content warnings</div>
      </div>
      <div
        style={{
          position: "absolute",
      ...
      >
        ...
        <div style={{ fontSize: 27, fontWeight: 600, color: T.success }}>Correct indexing across all 3 languages</div>
      </div>
    </AbsoluteFill>
  );
};
