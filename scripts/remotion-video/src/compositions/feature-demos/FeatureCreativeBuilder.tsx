 ...
 * must rebuild these numbers with it. RE-SHOOT (2026-09-20): same beats, same
 * windows, picture rewritten only.
 *
 * Art direction (STEP 0 of lux-batch-instructions.md), re-assigned this pass:
 *   archetype 2 "zoom-in" — beat 1 is a wide, small-scale view of the whole
 *   Telegram bot screen; each of beats 2-3 is a camera push into ONE part of
 *   that same screen. The camera then parks and the phone fades out — beats
 *   4-5 are the real product (STEP 0c, owner rule 2026-09-06): the commit
 *   that assembles the prompt, then the feature's own page.
 *   mood — mint (fresh green/teal), via MOODS.mint.
 *
 ...
 *
 * Beats 1-3 (invisible plumbing — a Telegram chat has no public webpage to
 * record) share one camera transform (`translate(tx,ty) scale(s)`) over a
 * single "world" div holding the phone screen at fixed coordinates; each
 * beat lives in its own non-overlapping y-band so nothing crossfades with
 * itself. That whole world fades out before beat 4 (STEP 0c: UI beats show
 * the REAL product). Beats 4-5 are LiveWindow recordings driven by
 * shots/p19.json: the real commit diff (b93ea22) that shows the assembled
 * prompt, then the feature's own write-up page as the payoff proof. Single
 * tech name in the whole clip: Azure OpenAI (one FilterChip, beat 4).
 */
...
import shots from "./shots/p19.json";
...
// ── The phone screen (beats 1-3 only) — one fixed layout in "world" coords ──
const PHONE_X = 360;
const PHONE_Y = 26;
const PHONE_W = 460;
const PHONE_H = 668;
...
// Camera targets: [center-x, center-y, scale]. A wide establishing shot, then
// two progressively tighter pushes into the phone. No pull-back move any
// more — beats 4-5 are screen-fixed LiveWindows, not part of this camera.
const CAM_F = [0, 178, 204, 352, 378, 551];
const CAM_CX = [590, 590, 590, 590, 590, 590];
const CAM_CY = [360, 360, 285, 285, 443, 443];
const CAM_S = [0.95, 0.95, 1.9, 1.9, 2.5, 2.5];
...
export const FeatureCreativeBuilder: React.FC = () => {
  ...
  // Fully gone by 562, clear of the gap before the live window opens at 568.
  // The whole "world" (phone + camera) fades on this same curve — the real
  // product takes over the screen for beats 4-5, nothing drawn survives it.
  const b3Out = 1 - seg(frame, 546, 562);
  const worldOpacity = b3Out;
  ...
  // ── Beat 4 — LIVE: the real commit that assembles one prompt ─────────────
  // Fully gone by 774 (b4's own end), clear of the gap before b5 opens at 783.
  const b4 = seg(frame, 568, 584) * (1 - seg(frame, 758, 774));

  // ── Beat 5 — LIVE: the payoff, the feature's own page ────────────────────
  // The window opens right at 783 and holds — no fade-out, nothing after it.
  const b5Win = seg(frame, 783, 799);
  ...
  return (
    <PaletteProvider value={P}>
      <div style={{ position: "absolute", inset: 0, fontFamily }}>
        ...
        {/* Small persistent eyebrow — screen-fixed, fades with the phone world */}
        <div
          style={{
            position: "absolute",
            left: 60,
            ...
            color: P.muted,
            opacity: seg(frame, 10, 26) * worldOpacity,
            fontFamily,
          }}
        >
          AI Creative Builder
        ...
        {/* ════ The world — one camera transform over the whole phone (beats 1-3) ════ */}
        <div
          style={{
            position: "absolute",
            left: 0,
        ...
        >
          {/* Phone shell — the one drawn element that survives beats 1-3 */}
        ...
        {/* ════ Beat 4 — LIVE: the commit that assembles the prompt ════ */}
        <div style={{ position: "absolute", left: 60, top: 44, opacity: b4, fontFamily }}>
          <div style={{ fontSize: 18, fontWeight: 700, letterSpacing: 1.6, textTransform: "uppercase", color: P.muted }}>
            Options tapped
          </div>
          <div style={{ fontSize: 84, fontWeight: 800, color: P.accent, lineHeight: 1, marginTop: 6 }}>42</div>
        </div>
        <div style={{ position: "absolute", left: 226, top: 90, width: 480, fontSize: 21, fontWeight: 700, color: P.ink, lineHeight: 1.3, opacity: b4, fontFamily }}>
          taps, assembled into one instruction
        </div>
        <FilterChip x={952} y={56} text="Azure OpenAI" icon="⚙" opacity={b4} color={P.accent} />
        <LiveWindow file={shots} shot="commit" title="github.com/…/commit/b93ea22" from={568} hold={206} opacity={b4} win={WIN_DEFAULT} />

        {/* ════ Beat 5 — LIVE: the feature's own page, the payoff ════ */}
        <LiveWindow file={shots} shot="page" title="vitalii.no/features/…-p19" from={783} hold={199} opacity={b5Win} win={{ x: 60, y: 196, w: 660, h: 450 }} />
        <Panel x={760} y={196} w={460} h={450} tone="success" opacity={cardIn}>
          <div style={{ padding: "30px 32px", fontFamily }}>
