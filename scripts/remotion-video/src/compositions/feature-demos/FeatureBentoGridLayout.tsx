/**
 * FeatureBentoGridLayout — feature p27 — 1280x720, 15s @ 30fps, silent, loop-friendly.
 *
 * Story: a linear scroll forces every visitor down the same path, hiding
 * what matters to them → BentoGrid (Next.js, CSS Grid) renders 6
 * independent BentoGridItems (About, Services, Projects, Skills, News,
 * Blog), each with its own theme.colors pair and a 700ms hover fill →
 * hover triggers HeroTextAnimation (GSAP liquid wave); 3+ seconds on
 * Projects explodes into a full grid → session duration up 30%.
 */
...
// ── Layout ──────────────────────────────────────────────────────────
const TOP = { x: 440, y: 50, w: 400, h: 88, label: "Linear scroll portfolio" };
const TOP_B: Pt = { x: TOP.x + TOP.w / 2, y: TOP.y + TOP.h };

const RIGHT1 = { x: 850, y: 210, w: 300, h: 78, label: "visitor decides nothing" };
const RIGHT1_T: Pt = { x: RIGHT1.x + RIGHT1.w / 2, y: RIGHT1.y };

const LEFT1 = { x: 130, y: 210, w: 280, h: 78, label: "BentoGrid (Next.js, CSS Grid)" };
...
export const FeatureBentoGridLayout: React.FC = () => {
  ...
  // ── Beat 1 (0–116): linear scroll forces one path, hiding what matters ──
  ...
  // ── Beat 2 (128–232): 6 BentoGridItems, each with a unique color pair ──
  ...
  // ── Beat 3 (246–340): hover -> liquid GSAP fill; 3s on Projects -> explosion grid ──
  ...
  return (
    <AbsoluteFill style={{ fontFamily }}>
      ...
      <SchemaNode {...RIGHT1} state="danger" lit={rightLit} opacity={rightOp} label={RIGHT1.label} fontSize={18}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>forced About → ... → Blog</div>
      </SchemaNode>
      <Badge x={RIGHT1.x + RIGHT1.w / 2 - 16} y={RIGHT1.y - 34} kind="cross" scale={xScale} opacity={xScale} size={32} />
      <Pill x={TOP.x - 40} y={TOP.y - 44} text="recruiters hunt for what matters to them" color={T.danger} opacity={pill1Op} fontSize={16} />

      <SchemaNode {...LEFT1} state="accent" lit={leftLit} opacity={leftOp} label={LEFT1.label} fontSize={16} />
      <Token pts={[TOP_B, LEFT1_T]} t={tCe} opacity={tCeVis * lf} />
      <SchemaNode {...MID} state="accent" lit={midLit} opacity={midOp} label="6 BentoGridItems" fontSize={20}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2, textAlign: "center" }}>About · Services · Projects · Skills · News · Blog</div>
      </SchemaNode>
      <Token pts={[LEFT1_B, MID_T]} t={tEf} opacity={tEfVis * lf} />

      <SchemaNode {...BOTLEFT} state="success" lit={botLeftLit} opacity={botLeftOp} label="HeroTextAnimation (GSAP)" fontSize={16}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>liquid wave, 700ms fill</div>
      </SchemaNode>
      <SchemaNode {...BOTRIGHT} state="success" lit={botRightLit} opacity={botRightOp} label="3s hover -> explosion grid" fontSize={18}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>Projects thumbnails, Framer Motion</div>
      </SchemaNode>
      ...
      <Pill x={BOTLEFT.x - 10} y={BOTLEFT.y + BOTLEFT.h + 14} text="40% opacity, 700ms transition per item" color={T.success} opacity={pill3Op} fontSize={15} />

      <Caption x={90} y={648} w={1100} text="A linear scroll forces every visitor down the same path, wasting their time" color={T.danger} opacity={cap1} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="Six independent BentoGridItems let visitors pick their own entry point" color={T.text} opacity={cap2} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="Hover triggers a liquid GSAP fill; 3+ seconds on Projects explodes into a grid" color={T.success} opacity={cap3} fontSize={22} weight={600} />

      <div style={{ position: "absolute", left: 0, top: 598, width: 1280, textAlign: "center", opacity: metricOp }}>
        <div style={{ fontSize: 22, fontWeight: 600, color: T.muted }}>Non-linear, self-directed exploration</div>
      </div>
      <div
        style={{
          position: "absolute",
      ...
      >
        ...
        <div style={{ fontSize: 27, fontWeight: 600, color: T.success }}>Session duration up 30%, bounce rate down</div>
      </div>
    </AbsoluteFill>
  );
};
