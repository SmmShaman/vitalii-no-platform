/**
 * FeatureDynamicOgImages — feature p36 — 1280x720, 15s @ 30fps, silent, loop-friendly.
 *
 * Story: manually designing a unique 16:9 OG image for 5-10 daily
 * articles was simply impossible → app/opengraph-image.tsx (Next.js
 * Edge Function) hits an internal API for title/category/section color
 * → Satori renders HTML/CSS to PNG: truncated title, category badge,
 * vitalii.no branding, gradient matching section color → cached at the
 * CDN edge on first request → 100% manual design work eliminated.
 */
...
// ── Layout ──────────────────────────────────────────────────────────
const TOP = { x: 440, y: 50, w: 400, h: 88, label: "5-10 articles published daily" };
const TOP_B: Pt = { x: TOP.x + TOP.w / 2, y: TOP.y + TOP.h };

const RIGHT1 = { x: 850, y: 210, w: 300, h: 78, label: "generic or missing OG image" };
const RIGHT1_T: Pt = { x: RIGHT1.x + RIGHT1.w / 2, y: RIGHT1.y };

const LEFT1 = { x: 130, y: 210, w: 280, h: 78, label: "app/opengraph-image.tsx" };
...
export const FeatureDynamicOgImages: React.FC = () => {
  ...
  // ── Beat 1 (0–116): generic/missing OG image kills social engagement ──
  ...
  // ── Beat 2 (128–232): Edge Function fetches metadata -> Satori renders PNG ──
  ...
  // ── Beat 3 (246–340): branded gradient PNG, cached at the CDN edge ──
  ...
  return (
    <AbsoluteFill style={{ fontFamily }}>
      ...
      <SchemaNode {...RIGHT1} state="danger" lit={rightLit} opacity={rightOp} label={RIGHT1.label} fontSize={17}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>killed social engagement</div>
      </SchemaNode>
      <Badge x={RIGHT1.x + RIGHT1.w / 2 - 16} y={RIGHT1.y - 34} kind="cross" scale={xScale} opacity={xScale} size={32} />
      <Pill x={TOP.x - 30} y={TOP.y - 44} text="manual design impossible at this volume" color={T.danger} opacity={pill1Op} fontSize={16} />
      ...
      <SchemaNode {...MID} state="accent" lit={midLit} opacity={midOp} label="Satori: HTML/CSS -> PNG" fontSize={19}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2, textAlign: "center" }}>fetches title, category, color</div>
      </SchemaNode>
      <Token pts={[LEFT1_B, MID_T]} t={tEf} opacity={tEfVis * lf} />

      <SchemaNode {...BOTLEFT} state="success" lit={botLeftLit} opacity={botLeftOp} label="title + category badge" fontSize={17}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>gradient matches section color</div>
      </SchemaNode>
      <SchemaNode {...BOTRIGHT} state="success" lit={botRightLit} opacity={botRightOp} label="Vercel CDN cache" fontSize={19}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>cached on first request</div>
      </SchemaNode>
      ...
      <Pill x={BOTLEFT.x - 10} y={BOTLEFT.y + BOTLEFT.h + 14} text="16:9, two-line truncated title" color={T.success} opacity={pill3Op} fontSize={15} />

      <Caption x={90} y={648} w={1100} text="Manually designing an OG image for 5-10 daily articles was simply impossible" color={T.danger} opacity={cap1} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="Satori renders a branded 16:9 PNG from live Supabase article metadata" color={T.text} opacity={cap2} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="The image is generated and cached at the CDN edge on first request" color={T.success} opacity={cap3} fontSize={22} weight={600} />

      <div style={{ position: "absolute", left: 0, top: 598, width: 1280, textAlign: "center", opacity: metricOp }}>
        <div style={{ fontSize: 22, fontWeight: 600, color: T.muted }}>100% manual design work eliminated</div>
      </div>
      <div
        style={{
          position: "absolute",
      ...
      >
        ...
        <div style={{ fontSize: 27, fontWeight: 600, color: T.success }}>Higher CTR and social engagement</div>
      </div>
    </AbsoluteFill>
  );
};
