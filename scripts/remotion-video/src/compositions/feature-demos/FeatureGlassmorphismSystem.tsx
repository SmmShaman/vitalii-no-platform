/**
 * FeatureGlassmorphismSystem — feature p34 — 1280x720, 15s @ 30fps, silent, loop-friendly.
 *
 * Story: 17+ admin components drifted slightly on every new feature —
 * different box-shadow, border-radius, opacity — a visually jarring,
 * amateurish system → a strict glassmorphism language, Tailwind
 * utility-only, replaces all custom CSS: bg-white/10 backdrop-blur-lg
 * rounded-xl border-white/20 → Framer Motion whileHover scale 1.05 and
 * standardized Loader2/error states everywhere → 100% visual consistency.
 */
...
// ── Layout ──────────────────────────────────────────────────────────
const TOP = { x: 440, y: 50, w: 400, h: 88, label: "17+ admin components" };
const TOP_B: Pt = { x: TOP.x + TOP.w / 2, y: TOP.y + TOP.h };

const RIGHT1 = { x: 850, y: 210, w: 300, h: 78, label: "drifting shadow / radius / opacity" };
const RIGHT1_T: Pt = { x: RIGHT1.x + RIGHT1.w / 2, y: RIGHT1.y };

const LEFT1 = { x: 130, y: 210, w: 280, h: 78, label: "Tailwind utility classes only" };
...
export const FeatureGlassmorphismSystem: React.FC = () => {
  ...
  // ── Beat 1 (0–116): unconscious drift makes the admin panel feel disjointed ──
  ...
  // ── Beat 2 (128–232): strict Tailwind-only glassmorphism replaces custom CSS ──
  ...
  // ── Beat 3 (246–340): Framer Motion micro-interactions, standardized states ──
  ...
  return (
    <AbsoluteFill style={{ fontFamily }}>
      ...
      <SchemaNode {...RIGHT1} state="danger" lit={rightLit} opacity={rightOp} label={RIGHT1.label} fontSize={16}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>visually jarring, amateurish</div>
      </SchemaNode>
      <Badge x={RIGHT1.x + RIGHT1.w / 2 - 16} y={RIGHT1.y - 34} kind="cross" scale={xScale} opacity={xScale} size={32} />
      <Pill x={TOP.x - 30} y={TOP.y - 44} text="each new feature drifted unconsciously" color={T.danger} opacity={pill1Op} fontSize={16} />
      ...
      <SchemaNode {...MID} state="accent" lit={midLit} opacity={midOp} label="bg-white/10 backdrop-blur-lg" fontSize={17}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2, textAlign: "center" }}>rounded-xl border-white/20</div>
      </SchemaNode>
      <Token pts={[LEFT1_B, MID_T]} t={tEf} opacity={tEfVis * lf} />

      <SchemaNode {...BOTLEFT} state="success" lit={botLeftLit} opacity={botLeftOp} label="whileHover scale 1.05" fontSize={17}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>motion.button</div>
      </SchemaNode>
      <SchemaNode {...BOTRIGHT} state="success" lit={botRightLit} opacity={botRightOp} label="Loader2 · red-500/10 errors" fontSize={17}>
        <div style={{ fontSize: 12, color: T.muted, fontWeight: 500, marginTop: 2 }}>standardized states</div>
      </SchemaNode>
      ...
      <Pill x={BOTLEFT.x - 10} y={BOTLEFT.y + BOTLEFT.h + 14} text="platform colors mapped in tailwind.config" color={T.success} opacity={pill3Op} fontSize={15} />

      <Caption x={90} y={648} w={1100} text="Slight drift in shadows, radii, and opacity made the admin panel feel disjointed" color={T.danger} opacity={cap1} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="A strict glassmorphism language, Tailwind-only, replaces all custom CSS" color={T.text} opacity={cap2} fontSize={22} weight={600} />
      <Caption x={90} y={648} w={1100} text="Framer Motion micro-interactions and standardized states, everywhere" color={T.success} opacity={cap3} fontSize={22} weight={600} />

      <div style={{ position: "absolute", left: 0, top: 598, width: 1280, textAlign: "center", opacity: metricOp }}>
        <div style={{ fontSize: 22, fontWeight: 600, color: T.muted }}>New UI dev time down 40-50%</div>
      </div>
      <div
        style={{
          position: "absolute",
      ...
      >
        ...
        <div style={{ fontSize: 27, fontWeight: 600, color: T.success }}>100% visual consistency across all components</div>
      </div>
    </AbsoluteFill>
  );
};
