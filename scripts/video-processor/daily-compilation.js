...
async function loadFromDraft(draftId) {
  ...
  // Phrase-level Visual Director output generated by the Nano agent (its own Claude
  // subscription, not the NVIDIA/Claude API calls in visual-director.js's aiDirectVisuals()).
  const visualDirectivesAi = draft.visual_directives_ai || null;
  ...
  console.log(`✅ Draft loaded: ${articles.length} articles, ${segmentScripts.length} scripts`
    + (visualDirectivesAi ? `, visual directives for ${visualDirectivesAi.length} segments` : ', NO visual directives'));
...
async function main() {
  ...
  // NVIDIA/Claude API chain (aiDirectVisuals()) for the pre-timing pass, but still runs the same
  // merge/fallback/variety pipeline (real timestamps only exist now, post-TTS).
  console.log(plan.visualDirectivesAi && plan.visualDirectivesAi.length > 0
