...
| **Edge Functions** | Deno | - |
| **AI (text)** | Per-task routing in `azure-to-gemini-shim.ts` (`llm_route`). **RULE (owner, 2026-07-25): rewrites and translations NEVER use Gemini — it is billed on this project.** **quality** (article rewrites EN/NO/UA, RSS rewrite in `website-publish`, `backfill-translations`, `regenerate-translations`) = gpt-oss-120b → gpt-oss-20b → Groq qwen3.6-27b → NVIDIA, **no Gemini in the chain**; **bulk** (pre-moderation, dup checks, RSS scoring) = qwen3.6-27b → gpt-oss-20b → Gemini → NVIDIA; **default** (teasers, image prompts, enrichment) = gpt-oss-120b → gpt-oss-20b → Groq qwen → Gemini → NVIDIA. Each Groq model has its own pool; gpt-oss are 8k TPM / 200k TPD each. **2026-08-19: Groq decommissioned ALL Llama chat models (llama-3.3-70b-versatile, llama-3.1-8b-instant, llama-4-scout → 404); qwen/qwen3.6-27b is the only other chat model — it is a thinking model, always pass `reasoning_format: 'hidden'` and strip `<think>` blocks.** `callLLM` in `gemini-llm.ts` = Groq only (GROQ_MODEL, default qwen3.6-27b) | - |
...
| **RSS gates** | `analyze-rss-article` applies BOTH `RSS_MIN_RELEVANCE_SCORE` (default 5, was hardcoded) and `MIN_QUALITY_SCORE` (default 6). Relevance = is it our topic; quality = does the text say anything. The RSS stream bypassed pre-moderation entirely before 2026-07-25 | - |
