 ...
 *
 * Two modes:
 *   A. AUTO (cron): auto_digest → LLM ranks articles + writes script → immediately dispatches
 *      auto_chain (scenario → images → render). Phrase-level visual direction is generated
 *      automatically at render time by directVisuals() in visual-director.js (NVIDIA NIM →
 *      Claude Sonnet fallback → heuristic fallback) — fully unattended, nothing to wait for.
 *   B. MANUAL: initiate_digest → moderator picks articles → generate_script → scenario → images → render
 *
 * Actions:
 *   - auto_digest              — LLM selects top-10 + writes script + dispatches auto_chain (skips manual selection)
 ...
 *   - submit_visual_directives — (legacy manual path) accepts precomputed visual directives and resumes the chain
 *   - generate_scenario        — AI plans visual scenario (article-level: headline/keyQuote/imageSearchQueries)
...
Return JSON: {"introScript": "Velkommen til dagens nyhetsdigest fra Vitalii Berbeha. ...", "outroScript": "Det var alt for i dag. Abonner på kanalen og trykk liker-knappen! ...", "introTranslationEn": "...", "outroTranslationEn": "..."}`,
  ...
  console.log(`✅ Auto digest: selected ${validSelectedIds.length}/${articles.length} articles`);

  // Fully automatic pipeline — resume immediately (scenario → images → render).
  // No precomputed visual_directives_ai is written here; directVisuals() in
  // visual-director.js generates it itself at render time (NVIDIA NIM → Claude
  // Sonnet fallback), and falls back further to heuristic directives if both LLM
  // calls fail — so there is nothing external to wait for. (The "Nano agent" this
  // used to pause for was never actually scheduled anywhere and left every draft
  // stuck at pending_visual_direction indefinitely.)
  const baseUrlAd = Deno.env.get("SUPABASE_URL")!;
  const srvKeyAd = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
  try {
    const chainRespAd = await fetch(`${baseUrlAd}/functions/v1/daily-video-bot?action=auto_chain&target_date=${date}`, {
      method: "POST",
      headers: { "Authorization": `Bearer ${srvKeyAd}`, "Content-Type": "application/json" },
      body: JSON.stringify({}),
    });
    console.log(`✅ auto_chain dispatched: HTTP ${chainRespAd.status}`);
  } catch (e: any) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    switch (action || body.action) {
      ...
      case "submit_visual_directives": {
        // Nano (agent, via its own Claude subscription) has generated phrase-level
        // visual direction for today's draft and is handing it back. Save it, then
        // resume the render chain (scenario → images → render) that autoDigest()
        // paused after creating the script draft.
        if (!targetDate) return json({ error: "target_date required" }, 400);
        if (!Array.isArray(body.visual_directives)) return json({ error: "visual_directives array required" }, 400);
        console.log(`🎨 Received visual directives from Nano for ${targetDate}: ${body.visual_directives.length} segments`);
        const { error: vdError } = await supabase
          .from("daily_video_drafts")
          .update({ visual_directives_ai: body.visual_directives })
          .eq("target_date", targetDate);
        if (vdError) return json({ error: `Failed to save visual_directives_ai: ${vdError.message}` }, 500);

        const baseUrlVd = Deno.env.get("SUPABASE_URL")!;
        const srvKeyVd = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
        try {
          const chainRespVd = await fetch(`${baseUrlVd}/functions/v1/daily-video-bot?action=auto_chain&target_date=${targetDate}`, {
            method: "POST",
            headers: { "Authorization": `Bearer ${srvKeyVd}`, "Content-Type": "application/json" },
            body: JSON.stringify({}),
          });
          console.log(`✅ auto_chain dispatched (post-visual-direction): HTTP ${chainRespVd.status}`);
        } catch (e: any) {
          console.error(`⚠️ Auto-chain dispatch failed after visual directives: ${e.message}`);
          await sendMessage(TELEGRAM_CHAT_ID, `⚠️ <b>Auto-chain dispatch failed after visual directives:</b> ${e.message}`);
