 ...
 *
 * Two modes:
 *   A. AUTO (cron): auto_digest → LLM ranks articles + writes script → pauses (pending_visual_direction)
 *      → Nano agent generates phrase-level visual direction via its own Claude subscription and
 *      calls submit_visual_directives → scenario → images → render
 ...
 *   - submit_visual_directives — Nano hands back phrase-level visual direction, resumes the chain
 ...
 *
 * Called by: telegram-webhook (callback handlers), cron workflow, manual, Nano agent (submit_visual_directives)
 */
...
Return JSON: {"introScript": "Velkommen til dagens nyhetsdigest fra Vitalii Berbeha. ...", "outroScript": "Det var alt for i dag. Abonner på kanalen og trykk liker-knappen! ...", "introTranslationEn": "...", "outroTranslationEn": "..."}`,
  ...
  // Pause here instead of auto-chaining to scenario/render. Phrase-level visual direction
  // (mood/transition/per-phrase image queries) is now generated by the Nano agent via its
  // Claude subscription — not the NVIDIA/Claude API calls in visual-director.js, which were
  // unreliable (deprecated model, then timeouts; Claude fallback had zero API credits).
  // Nano polls for status="pending_visual_direction" and POSTs back to
  // action=submit_visual_directives, which resumes the chain (scenario → images → render).
  await supabase
    .from("daily_video_drafts")
    .update({ status: "pending_visual_direction" })
    .eq("id", draft.id);
  console.log(`⏸ Draft ${draft.id} waiting for Nano to generate visual directives`);
